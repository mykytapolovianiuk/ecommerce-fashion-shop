...
const Shop: React.FC = () => {
  ...
  const constraints = useMemo(() => {
    ...
    // ВАЖЛИВО: без дефолтного orderBy('popularity').
    // Сортуємо лише коли активний фільтр ціни (буде потрібно створити індекс category+price asc)
    if (price) {
      c.push(where("price", ">=", price[0]));
      c.push(where("price", "<=", price[1]));
      // orderBy('price', 'asc') прибираємо з constraints, щоб не збирати індекс
      // Якщо хочеш сортування — поверни orderBy і створи індекс за лінком з консолі.
    }
  ...
  async function load(first = true) {
    ...
    try {
      ...
      const q = query(
        colRef,
        ...constraints,
        ...(first || !cursor ? [] : [startAfter(cursor)]),
        limit(PAGE)
      );
      ...
      setItems(first ? list : (prev) => [...prev, ...list]);
    } catch (e: any) {
      console.error("Firestore query error:", e?.message || e);
    } finally {
      setLoading(false);
    }
  }
  ...
  useEffect(() => {
    setCursor(null);
    setItems([]);   // скидаємо, щоб не міксувались результати
    load(true);
            ...
            <div className={styles.filters__chips}>
              {["S", "M", "L", "XL"].map((s) => (
                <button
                  ...
                  onClick={() => setSizes((v) => (v.includes(s) ? v.filter((i) => i !== s) : [...v, s]))}
                >
                  {s}
                </button>
              ))}
              ...
              {[
                ...
                <label key={r.join("-")} className={styles.filters__row}>
                  <input
                    type="radio"
                    name="price"
                    checked={price?.[0] === r[0] && price?.[1] === r[1]}
            ...
            <div className={styles.filters__tags}>
              {["hat", "shirt", "dress", "coat", "summer", "black"].map((t) => (
                <button
                  ...
                  onClick={() => setTags((v) => (v.includes(t) ? v.filter((i) => i !== t) : [...v, t]))}
                >
                  {t}
                </button>
              ))}
